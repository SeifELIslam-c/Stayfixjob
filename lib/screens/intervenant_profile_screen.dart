...
import 'package:stayfix_job/services/vps_media_service.dart';
...
class _IntervenantProfileScreenState extends State<IntervenantProfileScreen> {
  ...
  /// Returns true if at least one verification field is true (worth showing the card).
  static bool _hasAnyVerification({
    required Map<String, dynamic> quest,
    required bool cvReviewAuth,
  }) {
    if (cvReviewAuth) return true;
    if (quest['authorizedToWorkCanada'] == true) return true;
    if (quest['isAdult'] == true) return true;
    if (quest['noCriminalRecord'] == true) return true;
    if (quest['referredByEmployee'] == true) return true;
    return false;
  }
      ...
      slivers: [
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: _PrimaryBtn(onTap: _handleChoose),
          ),
        ),
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: _SecondaryBtn(onTap: _handleContact),
          ),
        ),
        ...
        const SliverToBoxAdapter(child: SizedBox(height: 16)),
        // Personal info card — skip if every visible field is empty
        if (name.isNotEmpty ||
            (data['email'] as String?)?.trim().isNotEmpty == true ||
            (data['phone'] as String?)?.trim().isNotEmpty == true ||
            address.isNotEmpty ||
            (data['dob'] as String?)?.trim().isNotEmpty == true) ...[
          SliverToBoxAdapter(
            child: _SectionCard(
        ...
        // Professional profile card — skip if all fields empty and no specialties
        if (displayRole.isNotEmpty ||
            expYears != null ||
            (data['jobLocation'] as String?)?.trim().isNotEmpty == true ||
            (data['jobAddress'] as String?)?.trim().isNotEmpty == true ||
            (data['jobStartDate'] as String?)?.trim().isNotEmpty == true ||
        ...
        // CV card — only if CV data exists
        if (cvBase64 != null && cvBase64.isNotEmpty) ...[
          SliverToBoxAdapter(
            child: _CvCard(
              cvBytes: cvBytes,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(LucideIcons.phone, size: 18, color: _kGold),
            const SizedBox(width: 10),
            Text(
              'Contacter',
              style: GoogleFonts.inter(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(LucideIcons.clock, color: _kGold, size: 16),
              const SizedBox(width: 8),
              Text(
                'Disponibilités',
                style: GoogleFonts.inter(
...
class _AvailPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = isGreen ? _kProfGreen : _kGold;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
      decoration: BoxDecoration(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(LucideIcons.languages, color: _kGold, size: 16),
              const SizedBox(width: 8),
              Text(
                'Langues',
                style: GoogleFonts.inter(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(LucideIcons.shieldCheck, color: _kGold, size: 16),
              const SizedBox(width: 8),
              Text(
                'Vérifications',
                style: GoogleFonts.inter(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              const Icon(LucideIcons.fileText, color: _kGold, size: 16),
              const SizedBox(width: 8),
              Text(
                'CV du candidat',
                style: GoogleFonts.inter(
